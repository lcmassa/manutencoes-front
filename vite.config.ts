        ...
        server.middlewares.use(async (req, res, next) => {
          ...
          if (req.url === '/token.jwt' || req.url === '/.iap-cli/token.jwt' || (req.url && req.url.startsWith('/token.jwt')) || (req.url && req.url.startsWith('/.iap-cli/token.jwt'))) {
            try {
              ...
              // Caminho onde o token realmente está (prioridade máxima)
              const tokenPathActual = path.join(homeDir, 'manutencoes/.iap-cli/token.jwt')
              // Caminho relativo: de manutencoes-front para manutencoes/.iap-cli
              const tokenPathRelativo = path.resolve(currentDir, '../.iap-cli/token.jwt')
              // Caminho absoluto esperado (fallback)
              const tokenPathPrimary = path.join(homeDir, 'PROJETOS/iap-apps/.iap-cli/token.jwt')
              const tokenPathWorkspaceFallback = path.join(homeDir, 'PROJETOS/.iap-cli/token.jwt')
              
              const possiblePaths = [
                tokenPathActual, // PRIORIDADE 1: token onde realmente está (manutencoes/.iap-cli)
                tokenPathRelativo, // PRIORIDADE 2: caminho relativo (fallback)
                tokenPathPrimary, // PRIORIDADE 3: token gerado por ./iap auth dentro de iap-apps/.iap-cli
                tokenPathWorkspaceFallback, // PRIORIDADE 4: token eventualmente salvo em PROJETOS/.iap-cli
              ]
              
              console.log(`[Vite] Procurando token em:`)
              console.log(`[Vite]   1. ${tokenPathActual}`)
              console.log(`[Vite]   2. ${tokenPathRelativo}`)
              console.log(`[Vite]   3. ${tokenPathPrimary}`)
              console.log(`[Vite]   4. ${tokenPathWorkspaceFallback}`)
